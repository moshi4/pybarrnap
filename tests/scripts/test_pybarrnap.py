...
def test_cli_with_min_option() -> None:
    """Test cli default run"""
    fasta_file = load_example_fasta_file("bacteria.fna")
    cmd = f"pybarrnap {fasta_file}"
    result = sp.run(cmd, shell=True, check=False)

    assert result.returncode == 0


def test_cli_with_gzip_file() -> None:
...
def test_cli_with_all_option(tmp_path: Path) -> None:
...
def test_cli_with_accurate_option(tmp_path: Path) -> None:
...
@skipif_cmscan_not_installed
def test_cli_with_accurate_all_option(tmp_path: Path) -> None:
...
def test_cli_with_invalid_option_value() -> None:
...
def test_cli_fasta_input_cases(tmp_path: Path) -> None:
...
def test_cli_version() -> None:
    """Test cli -v option"""
    result = sp.run(
        "pybarrnap -v", shell=True, capture_output=True, text=True, check=False
    )
